<template>
  <div class="main">
      ...
      <div class="main_content" v-if="reactivated">
          ...
          <div class="miro_answer">
              <div class="miro_holder v-step-0 v-step-1">
                  <iframe class="responsive" style="z-index: -1 !important" :src="src" frameBorder="0" scrolling="no" allowFullScreen></iframe>
              </div>
              <div :class="inputClass()+' v-step-9'" v-if="show_inputs">
                  <input v-model="userAnswer"  v-on:keyup.enter="validateAnswer" class="answer" @click="inputClicked" placeholder="Please enter your answer here 答えを入力してください" type="text">
                  <button @click="validateAnswer" class="submit">Submit</button>
              </div>
              <div class="answer_result_holder " v-if="show_result_holder">
                  <div v-if="answer_correct==false"  @click="show_result_holder=false" class="wrong_answer">Wrong answer. Please try again! 残念、不正解! もう一度挑戦しましょう!</div>
                  <div v-if="answer_correct" @click="reactivate" class="correct_answer animate__animated animate__rubberBand">{{correctMessage}}</div>

              </div>
          </div>
      </div>
      </transition>
      <div :class="[albumShow == true ? 'show':'', getPhotoAlbum()+' photo_album v-step-8']" @click.self="openAlbum()"  v-click-outside="closeAlbum">
          <button v-if="showControl" class="arrow_left" @click="prevPage()"></button>
          <div class="album_panel" @click.self="openAlbum()">
                <div class="album_section" @click.self="openAlbum()">
                   <div @click="openImage(0+pager)" :class="photodeg[degreeIndex()].first" v-if="album_image[currentIndex+0+pager]">
                        <img  class="album_img" :src="album_image[currentIndex+0+pager]"/>
                    </div>
                    <div @click="openImage(1+pager)" :class="photodeg[degreeIndex()].second" v-if="album_image[currentIndex+1+pager]">
                        <img  class="album_img" :src="album_image[currentIndex+1+pager]"/>
                    </div>
                </div>
                <div  class="album_section" @click.self="openAlbum()">
                   <div @click="openImage(2+pager)" :class="photodeg[degreeIndex()].third" v-if="album_image[currentIndex+2+pager]">
                        <img  class="album_img" :src="album_image[currentIndex+2+pager]"/>
                    </div>
                    <div @click="openImage(3+pager)" :class="photodeg[degreeIndex()].fourth" v-if="album_image[currentIndex+3+pager]">
                        <img  class="album_img" :src="album_image[currentIndex+3+pager]"/>
                    </div>
                </div>
          </div>
          ...
          <div class="album_panel" @click.self="openAlbum()">
                <div class="album_section" @click.self="openAlbum()">
                    <div @click="openImage(4+pager)" :class="photodeg[degreeIndex()].fifth" v-if="album_image[currentIndex+4+pager]">
                        <img  class="album_img" :src="album_image[currentIndex+4+pager]"/>
                    </div>
                    <div @click="openImage(5+pager)" :class="photodeg[degreeIndex()].sixth" v-if="album_image[currentIndex+5+pager]">
                        <img  class="album_img" :src="album_image[currentIndex+5+pager]"/>
                    </div>
                </div>
                <div class="album_section" @click.self="openAlbum()">
                    <div @click="openImage(6+pager)" :class="photodeg[degreeIndex()].seventh" v-if="album_image[currentIndex+6+pager]">
                        <img  class="album_img" :src="album_image[currentIndex+6+pager]"/>
                    </div>
                    <div @click="openImage(7+pager)" :class="photodeg[degreeIndex()].eight" v-if="album_image[currentIndex+7+pager]">
                        <img  class="album_img" :src="album_image[currentIndex+7+pager]"/>
                    </div>
                </div>
          </div>

          <button   v-if="showControl" class="arrow_right" @click="nextPage()"></button>
      ...
      <v-tour class="v_tour" name="myTour" :steps="steps"  :callbacks="myCallbacks" :options="{ highlight: true }">
...
let secondMiro = 'https://miro.com/app/live-embed/o9J_kgVl-yc=/?embedAutoplay=true&moveToViewport=-664,-510,1328,1018';
let codeResponse;
let teamSetup;
let tempPuzzleName;
...
export default {
    ...
    beforeMount(){
                ...
                else if(routename == 'karuizawa.index'){
                    // alert('test');
                     this.album_image = [
                ...
                else if(routename == 'sendai.index'){
                    this.album_image = [
                ...
                else if(routename == 'asakusa.index'){
                    this.album_image = [
    ...
    methods:{
        getStatus(){
                            ...
                            // console.log(response['data'][0].answered_current);
                               if(response['data'][0].answered_current == 1){
                                ...
                                if(response['data'][0].player_number != teamSetup.playerName){
                                        this.correctMessage = 'Your teammate got the correct answer! Click here to continue.'
            ...
            myCustomPreviousStepCallback (currentStep) {
                    if (currentStep === 7 || currentStep === 6 || currentStep === 5 || currentStep === 4) {
